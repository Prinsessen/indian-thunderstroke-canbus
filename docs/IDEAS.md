...
| B5 | **Power.** Corrected 2026-09-19: the CAN board is already on a fused feed — the DIAG service connector, pin A (`ENGBRK`, 12 V through the engine breaker, permanent), pin D ground, G/H CAN. What is left: (a) the **Keis clothing** from a fused accessory outlet (FR/REAR_12V_SWITCHED, or CHASSIS_TRUNK 14) instead of straight off the battery; (b) **DIAG pin B** (`VCMACC-6`, switched 12 V, on when the bike is awake) as a clean ignition-on signal for the board, one wire | ready | — | `WIRING-DIAGRAMS.md` §8 (DIAG table) |
