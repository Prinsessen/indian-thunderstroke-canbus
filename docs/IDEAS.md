...
| A2b | **The five display buttons reach the board without new wiring.** Chassis harness nets 204–207 and 145 run nacelle→rear (CHASSIS_FAIRING 8–11, 13 ↔ CHASSIS_TRUNK 8–11, 13) and are unused on a Springfield: jumper the cube's four button lines onto them at the front, pick them up at CHASSIS_TRUNK next to the board. Nothing imitates Ride Command; the board reads switches and emits `button` events. **How they get into the board:** (1) measure the five-way on the cube — resistor ladder (one analogue line + ground) or matrix (four digital lines); (2) rev 1.0 has no input header, but the review notes a stray pad on IO1 (ADC1-capable, `ACC_SENSE`, no track): a ladder can be soldered to it now with an external pull-up and clamp; TX/RX (GPIO43/44) give two digital inputs at most; a matrix waits for **rev 1.1 with a protected 8-way input header** (4 digital + 1 analogue + ground + 2 optocoupler outputs for the seat), designed after rev 1.0 bring-up; (3) firmware `keys.cpp` like `buttons.cpp`, 20 ms sampling, same `buttonEmit` path: `nav up/down/left/right/ok`, BLE codes 10–14, five rows in the app's table. Read only while awake; no RTC wake from the keys | ready | A3 (the cube and the pigtail); finding CHASSIS_TRUNK on the bike; a multimeter on the five-way (ladder or matrix) | `WIRING-DIAGRAMS.md` §6 |
...
| B5 | **Power.** Corrected 2026-09-19: the CAN board is already on a fused feed — the DIAG service connector, pin A (`ENGBRK`, 12 V through the engine breaker, permanent), pin D ground, G/H CAN. Corrected again the same day: **the Keis clothing stays on the battery** — the full kit (their strongest jacket, trousers, socks, gloves) draws well over 15 A, far beyond any accessory fuse on the bike, so an outlet is not an option. What is left: (a) **the kit's numbers (Keis' own spec sheets, 2026-09-19):** J601 jacket 65 W / 5.4 A, T103 trousers 39 W / 3.25 A, S302 socks 8 W / 0.6 A, G701 gloves 19.2 W / 1.6 A — **131 W, 10.9 A at 12 V; the elements are resistive, so at the bike's 14.4 V charging voltage that is about 13 A.** Under 15 A, but a 15 A blade fuse is only good for about 12 A continuous, so a single 15 A lead for the whole kit is marginal on a long cold ride. Either two fused leads (jacket + gloves ≈ 7 A, trousers + socks ≈ 4 A, which is how Keis' own Y-leads split it) or one 20 A lead on 2.5 mm² wire — **the lead already is 2.5 mm² (owner, 2026-09-19), so a 20 A blade fuse on it is the fix**, 2.5 mm² carries 20 A with a 0.3 V drop over the run. **Measured by the owner some time ago: 13.0–13.3 A with the engine running, all on HIGH** — exactly the 14.4 V figure, so the sheets were honest. Conclusion: 20 A blade fuse on the existing 2.5 mm² lead, done; the seat and the grips are on their own feeds and do not count here; (b) a **relay on the Keis lead driven from DIAG pin B** (`VCMACC-6`, switched 12 V) so the clothing lead is dead when the bike sleeps — the controllers draw standby current otherwise; (c) budget: the alternator is 56 A / 710 W at 2000 rpm (manual 10.7), the bike's own loads take their share, and the app already caps heat when the battery strains (`HeatCurve.cap`). CAN wake for the board stays as it is — it works; pin B is only a possible clean input, not a change | ready — swap the fuse | — | `WIRING-DIAGRAMS.md` §8 (DIAG table), manual 10.7 |
...
| D5 | **Two devices, one bike — done and tested on the road 2026-09-19** (phone ANOTHER DEVICE + X70 THIS DEVICE on firmware 2026.09.19-2: both connected, gestures clean, ignition cycle clean). Remaining: Play release 5 so the X70 gets the setting too. Original note: Firmware 2026.09.19-2 accepts two centrals (`BLE_MAX_CENTRALS`, per-link pairing state, advertising restarted after the first connect); app has "Clothing is controlled by: THIS DEVICE / ANOTHER DEVICE" in settings, which stops the phone in the pocket taking the Keis links. Test: two devices on the bench, ignition on, both get data, one owns the clothing, ignition off/on, nobody stuck. Original note: A phone in a pocket with the app running takes the only BLE connection to the bike and both Keis links, and the tablet on the bar gets nothing until the phone's Bluetooth is off (2026-09-19). Two halves: **firmware** — accept two centrals (NimBLE max connections, restart advertising in `onConnect` while below the limit, pairing state per connection instead of the single `gPaired`, notify all subscribers); **app** — a per-device setting for who controls the heated clothing, since a Keis controller talks to one client only, and a passive mode that never connects on its own. Until then: the device on the bar owns the bike, the phone stays off | ready to design | a bench test with two phones before the bike | app `README.md` "Two devices, one bike", `PROTOCOL.md` §6 |
